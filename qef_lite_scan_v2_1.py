...
# CORRECTION: Mock sentiment for error-free demo (no internet needed)
def mock_sentiment(idea_text):
    if 'demographic' in idea_text.lower() or 'bias' in idea_text.lower():
        # Simulates high bias score (0.92) for high-risk words
...
def QEF_Lite_Scan_v2_1(idea_text):
    ...
    if bias_sent[0]['score'] > 0.8:
        # Equity score drops significantly if bias confidence is high
...
# Expected Output: {'approved': False, 'reason': 'Low equity/veracity/impact', 'score': 0.64, 'resubmit': True}
